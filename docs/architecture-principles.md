...
* The three properties to preserve: behaviour has one home per screen, both tiers provably render the same data, and no rendition holds anything authoritative. The third was "views are stateless" until 2026-08-05, when the widget library gave views retained `Container`/`Label` instances to draw with. Holding *how* it draws is a view's business; holding *what is true* is not — a widget's position is a cache rebuilt from the model each frame, whereas a selected value would be an answer the other rendition could contradict. This is not MVC, MVP or MVVM and shouldn't be described as any of them (MVVM was considered and rejected: its value is binding and change notification, and Elite is immediate-mode with a full redraw per tick and polled input). The `Controller` suffix matches `PlanetController`/`AudioController` house style.
