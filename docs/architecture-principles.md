...
* Views live in a plugin assembly per tier — `EliteSharp.Views.EightBit/<Screen>View8Bit.cs` and `EliteSharp.Views.SixteenBit/<Screen>View16Bit.cs` — each referencing `EliteSharp.Abstractions` and nothing else, and neither referenced by the game (2026-08-03, see [decisions.md](decisions.md)). The game finds them in a `Views` folder beside the executable and takes the pack whose `Tier` matches, so no registration branches on the tier any more. The tier contract is a signature, not a convention: both views take the same model, so they can differ only in coordinates and font, and the formatting stays testable without a renderer.
* A pack hands over its screens as a `ViewSet`, keyed by the model each draws, and `ViewRegistry` checks that against the game's screens **once, at startup**. A missing screen or a missing pack is a startup failure that names what is missing; neither is something a commander should discover by opening a screen. What a view may touch of the game is `IViewSurface` — `Graphics`, `Layout`, `Palette` — and nothing else: `IEliteDraw` implements it, and everything past those three (projection, clipping, frame start/end) stays on the game's side of the seam.
...
* **Nothing that puts pixels on screen is shared between tiers** (maintainer, 2026-07-30), and this reaches past the screens. `Scanner`, `LaserDraw`, `GradientSun` and `StripedPlanet` each became a `<Name>Base` holding the logic with `<Name>8Bit`/`<Name>16Bit` holding the colours and coordinates; `EliteDraw` draws no chrome, and every screen is now a controller/model/view trio with one view per tier. The shape to copy is: **the base declares abstract members for every colour and every position, and holds no literal of either.** `ScannerBase` is the reference — ~20 offsets that were hardcoded to the 512x129 scanner are now named members, which is what let the 640-wide art be absorbed by editing one file.
* The exception is classes whose only colour is `White` and whose geometry is already derived (`Stars`, `BreakPattern`, `WireframePlanet`, the wireframe/solid suns, `WireframeRenderer`). Splitting those buys a class pair to hold one palette lookup that is identical on both tiers. Judgement, not an invariant — split them the moment they need to differ.
* `ViewLayout` is the single source of screen metrics: screen size, scanner size and coordinate scale in, everything else derived. Take metrics from `IEliteDraw.Layout`; do not add convenience mirrors of them elsewhere, which is what `IEliteDraw` accumulated before 2026-07-30 and what left every value with two spellings.

* `ViewLayout` describes **one region, the viewport** — everything above the HUD — and it starts at the screen origin (2026-07-31). It exposes no border and no scanner members: `BorderWidth` belongs to the base view that draws the frame, and the scanner's own geometry belongs to `ScannerBase`. This is what makes a position mean what it says: no border inset and no HUD edge enter the arithmetic, so the 8-bit viewport is exactly 40x25 of its 8x8 character cells and the border can change width without moving a single view. Screens that used to lay out from `ScannerLeft`/`Offset` lay out from `ViewportLeft`.
