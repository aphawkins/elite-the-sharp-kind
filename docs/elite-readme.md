...
Game settings are held in the `sharpkind.cfg` file, stored in JSON format, which should be in the same directory as the executable.  If the file is missing or invalid the game falls back to defaults.  It can take the following values:
