...
internal sealed class EquipmentView16Bit : BaseView16Bit, IView<EquipmentModel>
{
    ...
    public void Draw(EquipmentModel model)
    {
        ...
        foreach (EquipmentRow row in model.Rows)
        {
            ...
            {
                _draw.Graphics.DrawRectangleFilled(new(2 + _draw.Layout.Offset, y + 1), 508, 15, _colorLightRed);
            }
            ...
            {
                _draw.Graphics.DrawTextRight(new(450 + _draw.Layout.Offset, y), row.Price, nameof(FontType.Small), color);
            }

            y += 15;
        }

        _draw.Graphics.DrawTextLeft(new(16 + _draw.Layout.Offset, 340), model.Cash, nameof(FontType.Small), _colorWhite);
    }
}
