...
/// 40-character row. <see cref="IEliteDraw.DrawTextPretty"/> is deliberately
/// not used for that - it breaks even text that already fits, and it draws
/// left-aligned where these lines are centred.
/// </summary>
internal sealed class OptionsView8Bit : IView<OptionsModel>
{
    ...
    public void Draw(OptionsModel model)
    {
        ...
        _draw.DrawViewHeader("OPTIONS");

        for (int i = 0; i < model.Options.Count; i++)
        {
            Vector2 position = new(_draw.Centre.X - (OptionBarWidth / 2), FirstRowY + (i * RowSpacingY));
        ...
        float y = _draw.ScannerTop - CreditsBottomGapY - (lines.Count * CreditSpacingY);
