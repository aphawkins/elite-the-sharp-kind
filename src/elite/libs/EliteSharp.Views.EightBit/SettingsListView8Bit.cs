...
/// 40 columns, so both fit on one line. The shared settings controller steps
/// its cursor one row at a time to match, and the Back row sits near the foot
/// of the viewport rather than immediately under the list.
/// </para>
/// </summary>
internal sealed class SettingsListView8Bit : BaseView8Bit, IView<SettingsListModel>
{
    ...
    internal SettingsListView8Bit(IViewSurface surface)
        : base(surface)
    {
        _surface = surface;
    ...
    private void DrawBackRow(SettingsListModel model, int lastIndex)
    {
        ...
        {
            _surface.Graphics.DrawRectangleFilled(
                new(_surface.Layout.ViewportCentre.X - (Column(CellWidthColumns) / 2), Row(BackRow)),
                Column(CellWidthColumns),
                Row(CellHeightRows),
                _colorRed);
        }
