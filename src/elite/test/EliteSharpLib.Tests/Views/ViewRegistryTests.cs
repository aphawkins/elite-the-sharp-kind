...
public class ViewRegistryTests
{
    ...
    {
        // Arrange: a pack that draws everything but the market. The commander
        // must not find that out by opening the market.
        ShortPack pack = new(typeof(MarketModel));

        // Act & Assert
        InvalidOperationException ex = Assert.Throws<InvalidOperationException>(
            () => new ViewRegistry(pack, new FakeEliteDraw()));
    ...
    public void NamesEveryMissingScreenAtOnce()
    {
        ...
        ShortPack pack = new(typeof(MarketModel), typeof(QuitModel));
    ...
    {
        // Arrange
        ViewRegistry registry = new(new ShortPack(), new FakeEliteDraw());
    ...
    private sealed class ShortPack(params Type[] omitted) : IViewPack
    {
