...
namespace EliteSharp.Lasers
{
    internal sealed class LaserDraw
    {
        ...
        internal void DrawLaserLines()
        {
            ...
            {
                // Left laser
                _draw.Graphics.DrawTriangle(leftA, target, leftB, EColor.LighterRed);

                // Right laser
                _draw.Graphics.DrawTriangle(rightA, target, rightB, EColor.LighterRed);
            }
            else
            {
                // Left laser
                _draw.Graphics.DrawTriangleFilled(leftA, target, leftB, EColor.LighterRed);

                // Right laser
                _draw.Graphics.DrawTriangleFilled(rightA, target, rightB, EColor.LighterRed);
            }
        }
        ...
        internal void DrawLaserSights(LaserType laserType)
        {
            ...
            _draw.Graphics.DrawLine(new(x1 - 1, y1), new(x1 - 1, y2), EColor.LightGrey);
            _draw.Graphics.DrawLine(new(x1, y1), new(x1, y2), EColor.White);
            _draw.Graphics.DrawLine(new(x1 + 1, y1), new(x1 + 1, y2), EColor.LightGrey);
            ...
            _draw.Graphics.DrawLine(new(x1, y1 - 1), new(x2, y1 - 1), EColor.LightGrey);
            _draw.Graphics.DrawLine(new(x1, y1), new(x2, y1), EColor.White);
            _draw.Graphics.DrawLine(new(x1, y1 + 1), new(x2, y1 + 1), EColor.LightGrey);
